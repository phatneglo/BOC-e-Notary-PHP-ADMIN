 ...
 * Page class
 */
class PaymentTransactionsEdit extends PaymentTransactions
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PaymentTransactionsEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "PaymentTransactionsEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("transaction_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->transaction_id->setQueryStringValue($keyValue);
                $this->transaction_id->setOldValue($this->transaction_id->QueryStringValue);
            } elseif (Post("transaction_id") !== null) {
                $this->transaction_id->setFormValue(Post("transaction_id"));
                $this->transaction_id->setOldValue($this->transaction_id->FormValue);
            } else {
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->transaction_id->Visible && $this->transaction_id->Required) {
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (transaction_reference)
        if ($this->transaction_reference->CurrentValue != "") {
            $filterChk = "(\"transaction_reference\" = '" . AdjustSql($this->transaction_reference->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getEditRow($rsold)
    {
        ...
        // transaction_reference
        $this->transaction_reference->setDbValueDef($rsnew, $this->transaction_reference->CurrentValue, $this->transaction_reference->ReadOnly);
        ...
        // gateway_reference
        $this->gateway_reference->setDbValueDef($rsnew, $this->gateway_reference->CurrentValue, $this->gateway_reference->ReadOnly);

        // gateway_response
        $this->gateway_response->setDbValueDef($rsnew, $this->gateway_response->CurrentValue, $this->gateway_response->ReadOnly);
        ...
        // payment_receipt_url
        $this->payment_receipt_url->setDbValueDef($rsnew, $this->payment_receipt_url->CurrentValue, $this->payment_receipt_url->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['transaction_reference'])) { // transaction_reference
            $this->transaction_reference->CurrentValue = $row['transaction_reference'];
        }
        ...
        if (isset($row['gateway_reference'])) { // gateway_reference
            $this->gateway_reference->CurrentValue = $row['gateway_reference'];
        }
        if (isset($row['gateway_response'])) { // gateway_response
            $this->gateway_response->CurrentValue = $row['gateway_response'];
        }
        ...
        if (isset($row['payment_receipt_url'])) { // payment_receipt_url
            $this->payment_receipt_url->CurrentValue = $row['payment_receipt_url'];
        }
