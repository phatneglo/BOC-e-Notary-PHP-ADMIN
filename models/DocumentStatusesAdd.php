 ...
 * Page class
 */
class DocumentStatusesAdd extends DocumentStatuses
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentStatusesAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentStatusesAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("DocumentStatusesList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "DocumentStatusesList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "DocumentStatusesView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "DocumentStatusesList") {
                            ...
                            $returnUrl = "DocumentStatusesList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'status_name' first before field var 'x_status_name'
        $val = $CurrentForm->hasValue("status_name") ? $CurrentForm->getValue("status_name") : $CurrentForm->getValue("x_status_name");
        if (!$this->status_name->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->status_name->Visible = false; // Disable update for API request
            } else {
                $this->status_name->setFormValue($val);
            }
        }
    ...
    public function renderRow()
    {
        ...
        // status_name
        $this->status_name->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->status_name->EditValue = HtmlEncode($this->status_name->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->status_name->Visible && $this->status_name->Required) {
                if (!$this->status_name->IsDetailKey && EmptyValue($this->status_name->FormValue)) {
                    $this->status_name->addErrorMessage(str_replace("%s", $this->status_name->caption(), $this->status_name->RequiredErrorMessage));
                }
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->status_code->CurrentValue != "") { // Check field with unique index
            $filter = "(\"status_code\" = '" . AdjustSql($this->status_code->CurrentValue, $this->Dbid) . "')";
            $rsChk = $this->loadRs($filter)->fetch();
            if ($rsChk !== false) {
                $idxErrMsg = str_replace("%f", $this->status_code->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->status_code->CurrentValue, $idxErrMsg);
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['status_code'])) { // status_code
            $this->status_code->setFormValue($row['status_code']);
        }
        if (isset($row['status_name'])) { // status_name
            $this->status_name->setFormValue($row['status_name']);
        }
