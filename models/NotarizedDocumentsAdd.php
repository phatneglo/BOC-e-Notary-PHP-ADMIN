 ...
 * Page class
 */
class NotarizedDocumentsAdd extends NotarizedDocuments
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotarizedDocumentsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "NotarizedDocumentsAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "NotarizedDocumentsView") {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->doc_keycode->CurrentValue != "") { // Check field with unique index
            $filter = "(\"doc_keycode\" = '" . AdjustSql($this->doc_keycode->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getAddRow()
    {
        ...
        // page_number
        $this->page_number->setDbValueDef($rsnew, $this->page_number->CurrentValue, false);

        // book_number
        $this->book_number->setDbValueDef($rsnew, $this->book_number->CurrentValue, false);

        // series_of
        $this->series_of->setDbValueDef($rsnew, $this->series_of->CurrentValue, false);

        // doc_keycode
        $this->doc_keycode->setDbValueDef($rsnew, $this->doc_keycode->CurrentValue, false);

        // notary_location
        $this->notary_location->setDbValueDef($rsnew, $this->notary_location->CurrentValue, false);

        // notarization_date
        $this->notarization_date->setDbValueDef($rsnew, UnFormatDateTime($this->notarization_date->CurrentValue, $this->notarization_date->formatPattern()), false);

        // digital_signature
        $this->digital_signature->setDbValueDef($rsnew, $this->digital_signature->CurrentValue, false);

        // digital_seal
        $this->digital_seal->setDbValueDef($rsnew, $this->digital_seal->CurrentValue, false);

        // certificate_text
        $this->certificate_text->setDbValueDef($rsnew, $this->certificate_text->CurrentValue, false);

        // certificate_type
        $this->certificate_type->setDbValueDef($rsnew, $this->certificate_type->CurrentValue, false);

        // qr_code_path
        $this->qr_code_path->setDbValueDef($rsnew, $this->qr_code_path->CurrentValue, false);

        // notarized_document_path
        $this->notarized_document_path->setDbValueDef($rsnew, $this->notarized_document_path->CurrentValue, false);
        ...
        $this->revoked->setDbValueDef($rsnew, $tmpBool, strval($this->revoked->CurrentValue) == "");

        // revoked_at
        $this->revoked_at->setDbValueDef($rsnew, UnFormatDateTime($this->revoked_at->CurrentValue, $this->revoked_at->formatPattern()), false);

        // revoked_by
        $this->revoked_by->setDbValueDef($rsnew, $this->revoked_by->CurrentValue, false);

        // revocation_reason
        $this->revocation_reason->setDbValueDef($rsnew, $this->revocation_reason->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['document_number'])) { // document_number
            $this->document_number->setFormValue($row['document_number']);
        }
        if (isset($row['page_number'])) { // page_number
            $this->page_number->setFormValue($row['page_number']);
        }
        if (isset($row['book_number'])) { // book_number
            $this->book_number->setFormValue($row['book_number']);
        }
        if (isset($row['series_of'])) { // series_of
            $this->series_of->setFormValue($row['series_of']);
        }
        if (isset($row['doc_keycode'])) { // doc_keycode
            $this->doc_keycode->setFormValue($row['doc_keycode']);
        }
        if (isset($row['notary_location'])) { // notary_location
            $this->notary_location->setFormValue($row['notary_location']);
        }
        if (isset($row['notarization_date'])) { // notarization_date
            $this->notarization_date->setFormValue($row['notarization_date']);
        }
        ...
        if (isset($row['digital_seal'])) { // digital_seal
            $this->digital_seal->setFormValue($row['digital_seal']);
        }
        if (isset($row['certificate_text'])) { // certificate_text
            $this->certificate_text->setFormValue($row['certificate_text']);
        }
        if (isset($row['certificate_type'])) { // certificate_type
            $this->certificate_type->setFormValue($row['certificate_type']);
        }
        ...
        if (isset($row['notarized_document_path'])) { // notarized_document_path
            $this->notarized_document_path->setFormValue($row['notarized_document_path']);
        }
        if (isset($row['expires_at'])) { // expires_at
            $this->expires_at->setFormValue($row['expires_at']);
        }
        if (isset($row['revoked'])) { // revoked
            $this->revoked->setFormValue($row['revoked']);
        }
        if (isset($row['revoked_at'])) { // revoked_at
            $this->revoked_at->setFormValue($row['revoked_at']);
        }
        if (isset($row['revoked_by'])) { // revoked_by
            $this->revoked_by->setFormValue($row['revoked_by']);
        }
        if (isset($row['revocation_reason'])) { // revocation_reason
            $this->revocation_reason->setFormValue($row['revocation_reason']);
        }
    }
