 ...
 * Page class
 */
class DocumentVerificationAdd extends DocumentVerification
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentVerificationAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentVerificationAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "DocumentVerificationView") {
    ...
    {
        $this->failed_attempts->DefaultValue = $this->failed_attempts->getDefault(); // PHP
        $this->failed_attempts->OldValue = $this->failed_attempts->DefaultValue;
    }
    ...
    protected function getAddRow()
    {
        ...
        // notarized_id
        $this->notarized_id->setDbValueDef($rsnew, $this->notarized_id->CurrentValue, false);

        // document_number
        $this->document_number->setDbValueDef($rsnew, $this->document_number->CurrentValue, false);

        // keycode
        $this->keycode->setDbValueDef($rsnew, $this->keycode->CurrentValue, false);

        // verification_url
        $this->verification_url->setDbValueDef($rsnew, $this->verification_url->CurrentValue, false);
        ...
        // failed_attempts
        $this->failed_attempts->setDbValueDef($rsnew, $this->failed_attempts->CurrentValue, strval($this->failed_attempts->CurrentValue) == "");

        // blocked_until
        $this->blocked_until->setDbValueDef($rsnew, UnFormatDateTime($this->blocked_until->CurrentValue, $this->blocked_until->formatPattern()), false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['notarized_id'])) { // notarized_id
            $this->notarized_id->setFormValue($row['notarized_id']);
        }
        ...
        if (isset($row['keycode'])) { // keycode
            $this->keycode->setFormValue($row['keycode']);
        }
        if (isset($row['verification_url'])) { // verification_url
            $this->verification_url->setFormValue($row['verification_url']);
        }
        ...
        if (isset($row['failed_attempts'])) { // failed_attempts
            $this->failed_attempts->setFormValue($row['failed_attempts']);
        }
        if (isset($row['blocked_until'])) { // blocked_until
            $this->blocked_until->setFormValue($row['blocked_until']);
        }
    }
