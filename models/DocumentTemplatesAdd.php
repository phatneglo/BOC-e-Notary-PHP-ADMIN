 ...
 * Page class
 */
class DocumentTemplatesAdd extends DocumentTemplates
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DocumentTemplatesAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "DocumentTemplatesAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "DocumentTemplatesView") {
    ...
    {
        $this->version->DefaultValue = $this->version->getDefault(); // PHP
        $this->version->OldValue = $this->version->DefaultValue;
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->template_code->CurrentValue != "") { // Check field with unique index
            $filter = "(\"template_code\" = '" . AdjustSql($this->template_code->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getAddRow()
    {
        ...
        // template_name
        $this->template_name->setDbValueDef($rsnew, $this->template_name->CurrentValue, false);

        // template_code
        $this->template_code->setDbValueDef($rsnew, $this->template_code->CurrentValue, false);
        ...
        // html_content
        $this->html_content->setDbValueDef($rsnew, $this->html_content->CurrentValue, false);
        ...
        // version
        $this->version->setDbValueDef($rsnew, $this->version->CurrentValue, strval($this->version->CurrentValue) == "");
        ...
        $this->notary_required->setDbValueDef($rsnew, $tmpBool, strval($this->notary_required->CurrentValue) == "");

        // fee_amount
        $this->fee_amount->setDbValueDef($rsnew, $this->fee_amount->CurrentValue, false);

        // approval_workflow
        $this->approval_workflow->setDbValueDef($rsnew, $this->approval_workflow->CurrentValue, false);

        // template_type
        $this->template_type->setDbValueDef($rsnew, $this->template_type->CurrentValue, false);

        // header_text
        $this->header_text->setDbValueDef($rsnew, $this->header_text->CurrentValue, false);

        // footer_text
        $this->footer_text->setDbValueDef($rsnew, $this->footer_text->CurrentValue, false);

        // preview_image_path
        $this->preview_image_path->setDbValueDef($rsnew, $this->preview_image_path->CurrentValue, false);
        ...
        $this->is_system->setDbValueDef($rsnew, $tmpBool, strval($this->is_system->CurrentValue) == "");

        // owner_id
        $this->owner_id->setDbValueDef($rsnew, $this->owner_id->CurrentValue, false);

        // original_template_id
        $this->original_template_id->setDbValueDef($rsnew, $this->original_template_id->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['template_name'])) { // template_name
            $this->template_name->setFormValue($row['template_name']);
        }
        if (isset($row['template_code'])) { // template_code
            $this->template_code->setFormValue($row['template_code']);
        }
        ...
        if (isset($row['html_content'])) { // html_content
            $this->html_content->setFormValue($row['html_content']);
        }
        ...
        if (isset($row['notary_required'])) { // notary_required
            $this->notary_required->setFormValue($row['notary_required']);
        }
        ...
        if (isset($row['approval_workflow'])) { // approval_workflow
            $this->approval_workflow->setFormValue($row['approval_workflow']);
        }
        if (isset($row['template_type'])) { // template_type
            $this->template_type->setFormValue($row['template_type']);
        }
        if (isset($row['header_text'])) { // header_text
            $this->header_text->setFormValue($row['header_text']);
        }
        if (isset($row['footer_text'])) { // footer_text
            $this->footer_text->setFormValue($row['footer_text']);
        }
        if (isset($row['preview_image_path'])) { // preview_image_path
            $this->preview_image_path->setFormValue($row['preview_image_path']);
        }
        if (isset($row['is_system'])) { // is_system
            $this->is_system->setFormValue($row['is_system']);
        }
        if (isset($row['owner_id'])) { // owner_id
            $this->owner_id->setFormValue($row['owner_id']);
        }
        if (isset($row['original_template_id'])) { // original_template_id
            $this->original_template_id->setFormValue($row['original_template_id']);
        }
    }
