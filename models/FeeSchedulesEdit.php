 ...
 * Page class
 */
class FeeSchedulesEdit extends FeeSchedules
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FeeSchedulesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "FeeSchedulesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("fee_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->fee_id->setQueryStringValue($keyValue);
                $this->fee_id->setOldValue($this->fee_id->QueryStringValue);
            } elseif (Post("fee_id") !== null) {
                $this->fee_id->setFormValue(Post("fee_id"));
                $this->fee_id->setOldValue($this->fee_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->fee_id->IsDetailKey) {
            $this->fee_id->setFormValue($val);
        }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->fee_id->Visible && $this->fee_id->Required) {
                if (!$this->fee_id->IsDetailKey && EmptyValue($this->fee_id->FormValue)) {
                    $this->fee_id->addErrorMessage(str_replace("%s", $this->fee_id->caption(), $this->fee_id->RequiredErrorMessage));
                }
            }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // fee_name
        $this->fee_name->setDbValueDef($rsnew, $this->fee_name->CurrentValue, $this->fee_name->ReadOnly);

        // fee_amount
        $this->fee_amount->setDbValueDef($rsnew, $this->fee_amount->CurrentValue, $this->fee_amount->ReadOnly);

        // fee_type
        $this->fee_type->setDbValueDef($rsnew, $this->fee_type->CurrentValue, $this->fee_type->ReadOnly);

        // currency
        $this->currency->setDbValueDef($rsnew, $this->currency->CurrentValue, $this->currency->ReadOnly);

        // effective_from
        $this->effective_from->setDbValueDef($rsnew, UnFormatDateTime($this->effective_from->CurrentValue, $this->effective_from->formatPattern()), $this->effective_from->ReadOnly);

        // effective_to
        $this->effective_to->setDbValueDef($rsnew, UnFormatDateTime($this->effective_to->CurrentValue, $this->effective_to->formatPattern()), $this->effective_to->ReadOnly);
