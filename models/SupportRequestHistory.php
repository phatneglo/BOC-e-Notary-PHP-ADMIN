...
/**
 * Table class for support_request_history
 */
class SupportRequestHistory extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "support_request_history";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        // history_id
        $this->history_id = new DbField(
            $this, // Table
            'x_history_id', // Variable name
            'history_id', // Name
            '"history_id"', // Expression
            'CAST("history_id" AS varchar(255))', // Basic search expression
            3, // Type
            0, // Size
            -1, // Date/Time format
            false, // Is upload field
            '"history_id"', // Virtual expression
            false, // Is virtual
        ...
        $this->history_id->InputTextType = "text";
        $this->history_id->Raw = true;
        $this->history_id->IsAutoIncrement = true; // Autoincrement field
        $this->history_id->IsPrimaryKey = true; // Primary key field
        $this->history_id->Nullable = false; // NOT NULL field
        $this->history_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->history_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['history_id'] = &$this->history_id;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "support_request_history";
    }
    ...
    public function insert(&$rs)
    {
        ...
        try {
            ...
            $result = $conn->executeQuery(
                $queryBuilder->getSQL() . " RETURNING history_id",
        ...
        if ($result) {
            $this->history_id->setDbValue($result);
            $rs['history_id'] = $this->history_id->DbValue;
        }
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['history_id']) && !EmptyValue($this->history_id->CurrentValue)) {
                $rs['history_id'] = $this->history_id->CurrentValue;
            }
        }
        return $success;
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('history_id', $rs)) {
                AddFilter($where, QuotedName('history_id', $this->Dbid) . '=' . QuotedValue($rs['history_id'], $this->history_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->history_id->DbValue = $row['history_id'];
    ...
    {
        $keys = [];
        $val = $current ? $this->history_id->CurrentValue : $this->history_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->history_id->CurrentValue = $keys[0];
            } else {
                $this->history_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('history_id', $row) ? $row['history_id'] : null;
        } else {
            $val = !EmptyValue($this->history_id->OldValue) && !$current ? $this->history_id->OldValue : $this->history_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@history_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SupportRequestHistoryList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "SupportRequestHistoryView" => $Language->phrase("View"),
            "SupportRequestHistoryEdit" => $Language->phrase("Edit"),
            "SupportRequestHistoryAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "SupportRequestHistoryList";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "SupportRequestHistoryView",
            Config("API_ADD_ACTION") => "SupportRequestHistoryAdd",
            Config("API_EDIT_ACTION") => "SupportRequestHistoryEdit",
            Config("API_DELETE_ACTION") => "SupportRequestHistoryDelete",
            Config("API_LIST_ACTION") => "SupportRequestHistoryList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SupportRequestHistoryView", $parm);
        } else {
            $url = $this->keyUrl("SupportRequestHistoryView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "SupportRequestHistoryAdd?" . $parm;
        } else {
            $url = "SupportRequestHistoryAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SupportRequestHistoryEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SupportRequestHistoryList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SupportRequestHistoryAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SupportRequestHistoryList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("SupportRequestHistoryDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"history_id\":" . VarToJson($this->history_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->history_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->history_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("history_id") ?? Route("history_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->history_id->CurrentValue = $key;
            } else {
                $this->history_id->OldValue = $key;
            }
    ...
    {
        global $Response;
        $listPage = "SupportRequestHistoryList";
    ...
    public function renderEditRow()
    {
        ...
        // history_id
        $this->history_id->setupEditAttributes();
        $this->history_id->EditValue = $this->history_id->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->history_id);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->history_id);
