 ...
 * Page class
 */
class NotarizationRequestsAdd extends NotarizationRequests
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotarizationRequestsAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "NotarizationRequestsAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "NotarizationRequestsView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->priority->DefaultValue = $this->priority->getDefault(); // PHP
        $this->priority->OldValue = $this->priority->DefaultValue;
        $this->payment_status->DefaultValue = $this->payment_status->getDefault(); // PHP
        $this->payment_status->OldValue = $this->payment_status->DefaultValue;
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->request_reference->CurrentValue != "") { // Check field with unique index
            $filter = "(\"request_reference\" = '" . AdjustSql($this->request_reference->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getAddRow()
    {
        ...
        // request_reference
        $this->request_reference->setDbValueDef($rsnew, $this->request_reference->CurrentValue, false);

        // status
        $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, strval($this->status->CurrentValue) == "");

        // requested_at
        $this->requested_at->setDbValueDef($rsnew, UnFormatDateTime($this->requested_at->CurrentValue, $this->requested_at->formatPattern()), false);

        // notary_id
        $this->notary_id->setDbValueDef($rsnew, $this->notary_id->CurrentValue, false);

        // assigned_at
        $this->assigned_at->setDbValueDef($rsnew, UnFormatDateTime($this->assigned_at->CurrentValue, $this->assigned_at->formatPattern()), false);

        // notarized_at
        $this->notarized_at->setDbValueDef($rsnew, UnFormatDateTime($this->notarized_at->CurrentValue, $this->notarized_at->formatPattern()), false);

        // rejection_reason
        $this->rejection_reason->setDbValueDef($rsnew, $this->rejection_reason->CurrentValue, false);

        // rejected_at
        $this->rejected_at->setDbValueDef($rsnew, UnFormatDateTime($this->rejected_at->CurrentValue, $this->rejected_at->formatPattern()), false);

        // rejected_by
        $this->rejected_by->setDbValueDef($rsnew, $this->rejected_by->CurrentValue, false);

        // priority
        $this->priority->setDbValueDef($rsnew, $this->priority->CurrentValue, strval($this->priority->CurrentValue) == "");

        // payment_status
        $this->payment_status->setDbValueDef($rsnew, $this->payment_status->CurrentValue, strval($this->payment_status->CurrentValue) == "");

        // payment_transaction_id
        $this->payment_transaction_id->setDbValueDef($rsnew, $this->payment_transaction_id->CurrentValue, false);

        // modified_at
        $this->modified_at->setDbValueDef($rsnew, UnFormatDateTime($this->modified_at->CurrentValue, $this->modified_at->formatPattern()), false);

        // ip_address
        $this->ip_address->setDbValueDef($rsnew, $this->ip_address->CurrentValue, false);

        // browser_info
        $this->browser_info->setDbValueDef($rsnew, $this->browser_info->CurrentValue, false);

        // device_info
        $this->device_info->setDbValueDef($rsnew, $this->device_info->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['payment_transaction_id'])) { // payment_transaction_id
            $this->payment_transaction_id->setFormValue($row['payment_transaction_id']);
        }
        if (isset($row['modified_at'])) { // modified_at
            $this->modified_at->setFormValue($row['modified_at']);
        }
        if (isset($row['ip_address'])) { // ip_address
            $this->ip_address->setFormValue($row['ip_address']);
        }
        if (isset($row['browser_info'])) { // browser_info
            $this->browser_info->setFormValue($row['browser_info']);
        }
        if (isset($row['device_info'])) { // device_info
            $this->device_info->setFormValue($row['device_info']);
        }
    }
