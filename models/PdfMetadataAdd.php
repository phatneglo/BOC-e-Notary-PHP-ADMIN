 ...
 * Page class
 */
class PdfMetadataAdd extends PdfMetadata
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PdfMetadataAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "PdfMetadataAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "PdfMetadataView") {
    ...
    protected function getAddRow()
    {
        ...
        // pdf_type
        $this->pdf_type->setDbValueDef($rsnew, $this->pdf_type->CurrentValue, false);
        ...
        // page_count
        $this->page_count->setDbValueDef($rsnew, $this->page_count->CurrentValue, false);

        // generated_at
        $this->generated_at->setDbValueDef($rsnew, UnFormatDateTime($this->generated_at->CurrentValue, $this->generated_at->formatPattern()), false);

        // generated_by
        $this->generated_by->setDbValueDef($rsnew, $this->generated_by->CurrentValue, false);
        ...
        $this->is_final->setDbValueDef($rsnew, $tmpBool, strval($this->is_final->CurrentValue) == "");

        // processing_options
        $this->processing_options->setDbValueDef($rsnew, $this->processing_options->CurrentValue, false);
        return $rsnew;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['pdf_type'])) { // pdf_type
            $this->pdf_type->setFormValue($row['pdf_type']);
        }
        ...
        if (isset($row['page_count'])) { // page_count
            $this->page_count->setFormValue($row['page_count']);
        }
        if (isset($row['generated_at'])) { // generated_at
            $this->generated_at->setFormValue($row['generated_at']);
        }
        if (isset($row['generated_by'])) { // generated_by
            $this->generated_by->setFormValue($row['generated_by']);
        }
        if (isset($row['expires_at'])) { // expires_at
            $this->expires_at->setFormValue($row['expires_at']);
        }
        if (isset($row['is_final'])) { // is_final
            $this->is_final->setFormValue($row['is_final']);
        }
        if (isset($row['processing_options'])) { // processing_options
            $this->processing_options->setFormValue($row['processing_options']);
        }
    }
