...
class DocumentsAdd extends Documents
{
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->document_reference->CurrentValue != "") { // Check field with unique index
            $filter = "(\"document_reference\" = '" . AdjustSql($this->document_reference->CurrentValue, $this->Dbid) . "')";
    ...
    protected function getAddRow()
    {
        ...
        // document_title
        $this->document_title->setDbValueDef($rsnew, $this->document_title->CurrentValue, false);

        // document_reference
        $this->document_reference->setDbValueDef($rsnew, $this->document_reference->CurrentValue, false);
        ...
        // submitted_at
        $this->submitted_at->setDbValueDef($rsnew, UnFormatDateTime($this->submitted_at->CurrentValue, $this->submitted_at->formatPattern()), false);

        // company_name
        $this->company_name->setDbValueDef($rsnew, $this->company_name->CurrentValue, false);

        // customs_entry_number
        $this->customs_entry_number->setDbValueDef($rsnew, $this->customs_entry_number->CurrentValue, false);

        // date_of_entry
        $this->date_of_entry->setDbValueDef($rsnew, UnFormatDateTime($this->date_of_entry->CurrentValue, $this->date_of_entry->formatPattern()), false);

        // document_html
        $this->document_html->setDbValueDef($rsnew, $this->document_html->CurrentValue, false);

        // document_data
        $this->document_data->setDbValueDef($rsnew, $this->document_data->CurrentValue, false);
        ...
        $this->is_deleted->setDbValueDef($rsnew, $tmpBool, strval($this->is_deleted->CurrentValue) == "");

        // deletion_date
        $this->deletion_date->setDbValueDef($rsnew, UnFormatDateTime($this->deletion_date->CurrentValue, $this->deletion_date->formatPattern()), false);

        // deleted_by
        $this->deleted_by->setDbValueDef($rsnew, $this->deleted_by->CurrentValue, false);

        // parent_document_id
        $this->parent_document_id->setDbValueDef($rsnew, $this->parent_document_id->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['document_title'])) { // document_title
            $this->document_title->setFormValue($row['document_title']);
        }
        if (isset($row['document_reference'])) { // document_reference
            $this->document_reference->setFormValue($row['document_reference']);
        }
        ...
        if (isset($row['submitted_at'])) { // submitted_at
            $this->submitted_at->setFormValue($row['submitted_at']);
        }
        ...
        if (isset($row['customs_entry_number'])) { // customs_entry_number
            $this->customs_entry_number->setFormValue($row['customs_entry_number']);
        }
        if (isset($row['date_of_entry'])) { // date_of_entry
            $this->date_of_entry->setFormValue($row['date_of_entry']);
        }
        if (isset($row['document_html'])) { // document_html
            $this->document_html->setFormValue($row['document_html']);
        }
        if (isset($row['document_data'])) { // document_data
            $this->document_data->setFormValue($row['document_data']);
        }
        if (isset($row['is_deleted'])) { // is_deleted
            $this->is_deleted->setFormValue($row['is_deleted']);
        }
        if (isset($row['deletion_date'])) { // deletion_date
            $this->deletion_date->setFormValue($row['deletion_date']);
        }
        if (isset($row['deleted_by'])) { // deleted_by
            $this->deleted_by->setFormValue($row['deleted_by']);
        }
        if (isset($row['parent_document_id'])) { // parent_document_id
            $this->parent_document_id->setFormValue($row['parent_document_id']);
        }
