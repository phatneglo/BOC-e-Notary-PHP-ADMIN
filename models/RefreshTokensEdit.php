 ...
 * Page class
 */
class RefreshTokensEdit extends RefreshTokens
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RefreshTokensEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "RefreshTokensEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("token_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->token_id->setQueryStringValue($keyValue);
                $this->token_id->setOldValue($this->token_id->QueryStringValue);
            } elseif (Post("token_id") !== null) {
                $this->token_id->setFormValue(Post("token_id"));
                $this->token_id->setOldValue($this->token_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("token_id") ?? Route("token_id")) !== null) {
                    ...
                    $this->token_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("RefreshTokensList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'token_id' first before field var 'x_token_id'
        $val = $CurrentForm->hasValue("token_id") ? $CurrentForm->getValue("token_id") : $CurrentForm->getValue("x_token_id");
        if (!$this->token_id->IsDetailKey) {
            $this->token_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->token_id->CurrentValue = $this->token_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->token_id->Visible && $this->token_id->Required) {
                if (!$this->token_id->IsDetailKey && EmptyValue($this->token_id->FormValue)) {
                    $this->token_id->addErrorMessage(str_replace("%s", $this->token_id->caption(), $this->token_id->RequiredErrorMessage));
                }
            }
