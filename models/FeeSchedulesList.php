 ...
 * Page class
 */
class FeeSchedulesList extends FeeSchedules
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "FeeSchedulesList";
    ...
    public $FormName = "ffee_scheduleslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "FeeSchedulesList";
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "FeeSchedulesAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "FeeSchedulesDelete";
        $this->MultiUpdateUrl = "FeeSchedulesUpdate";
    ...
    public function run()
    {
        ...
        if ($this->IsModal) {
            $this->FormName = "ffee_schedulesgrid";
        }
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("ffee_schedulessrch");
        }
        $filterList = Concat($filterList, $this->fee_id->AdvancedSearch->toJson(), ","); // Field fee_id
        $filterList = Concat($filterList, $this->template_id->AdvancedSearch->toJson(), ","); // Field template_id
        $filterList = Concat($filterList, $this->fee_name->AdvancedSearch->toJson(), ","); // Field fee_name
        $filterList = Concat($filterList, $this->fee_amount->AdvancedSearch->toJson(), ","); // Field fee_amount
        $filterList = Concat($filterList, $this->fee_type->AdvancedSearch->toJson(), ","); // Field fee_type
        $filterList = Concat($filterList, $this->currency->AdvancedSearch->toJson(), ","); // Field currency
        $filterList = Concat($filterList, $this->effective_from->AdvancedSearch->toJson(), ","); // Field effective_from
        $filterList = Concat($filterList, $this->effective_to->AdvancedSearch->toJson(), ","); // Field effective_to
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("ffee_schedulessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field fee_name
        $this->fee_name->AdvancedSearch->SearchValue = @$filter["x_fee_name"];
        $this->fee_name->AdvancedSearch->SearchOperator = @$filter["z_fee_name"];
        $this->fee_name->AdvancedSearch->SearchCondition = @$filter["v_fee_name"];
        $this->fee_name->AdvancedSearch->SearchValue2 = @$filter["y_fee_name"];
        $this->fee_name->AdvancedSearch->SearchOperator2 = @$filter["w_fee_name"];
        $this->fee_name->AdvancedSearch->save();
        ...
        // Field fee_type
        $this->fee_type->AdvancedSearch->SearchValue = @$filter["x_fee_type"];
        $this->fee_type->AdvancedSearch->SearchOperator = @$filter["z_fee_type"];
        $this->fee_type->AdvancedSearch->SearchCondition = @$filter["v_fee_type"];
        $this->fee_type->AdvancedSearch->SearchValue2 = @$filter["y_fee_type"];
        $this->fee_type->AdvancedSearch->SearchOperator2 = @$filter["w_fee_type"];
        $this->fee_type->AdvancedSearch->save();
        ...
        $this->effective_from->AdvancedSearch->SearchValue = @$filter["x_effective_from"];
        $this->effective_from->AdvancedSearch->SearchOperator = @$filter["z_effective_from"];
        $this->effective_from->AdvancedSearch->SearchCondition = @$filter["v_effective_from"];
        $this->effective_from->AdvancedSearch->SearchValue2 = @$filter["y_effective_from"];
        $this->effective_from->AdvancedSearch->SearchOperator2 = @$filter["w_effective_from"];
        $this->effective_from->AdvancedSearch->save();

        // Field effective_to
        $this->effective_to->AdvancedSearch->SearchValue = @$filter["x_effective_to"];
        $this->effective_to->AdvancedSearch->SearchOperator = @$filter["z_effective_to"];
        $this->effective_to->AdvancedSearch->SearchCondition = @$filter["v_effective_to"];
        $this->effective_to->AdvancedSearch->SearchValue2 = @$filter["y_effective_to"];
        $this->effective_to->AdvancedSearch->SearchOperator2 = @$filter["w_effective_to"];
        $this->effective_to->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->fee_name;
        $searchFlds[] = &$this->fee_type;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->fee_name); // fee_name
            $this->updateSort($this->fee_amount); // fee_amount
            $this->updateSort($this->fee_type); // fee_type
            $this->updateSort($this->currency); // currency
            $this->updateSort($this->effective_from); // effective_from
            $this->updateSort($this->effective_to); // effective_to
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->fee_name->setSort("");
                $this->fee_amount->setSort("");
                $this->fee_type->setSort("");
                $this->currency->setSort("");
                $this->effective_from->setSort("");
                $this->effective_to->setSort("");
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"fee_schedules\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"fee_schedules\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"fee_schedules\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ffee_scheduleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"ffee_scheduleslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->fee_id->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"fee_schedules\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "fee_id");
            $this->createColumnOption($option, "template_id");
            $this->createColumnOption($option, "fee_name");
            $this->createColumnOption($option, "fee_amount");
            $this->createColumnOption($option, "fee_type");
            $this->createColumnOption($option, "currency");
            $this->createColumnOption($option, "effective_from");
            $this->createColumnOption($option, "effective_to");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"ffee_schedulessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"ffee_schedulessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="ffee_scheduleslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_fee_schedules", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_fee_schedules",
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->effective_from->TooltipValue = "";

            // effective_to
            $this->effective_to->HrefValue = "";
            $this->effective_to->TooltipValue = "";
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"ffee_schedulessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
