 ...
 * Page class
 */
class SupportRequestHistoryEdit extends SupportRequestHistory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SupportRequestHistoryEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "SupportRequestHistoryEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("history_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->history_id->setQueryStringValue($keyValue);
                $this->history_id->setOldValue($this->history_id->QueryStringValue);
            } elseif (Post("history_id") !== null) {
                $this->history_id->setFormValue(Post("history_id"));
                $this->history_id->setOldValue($this->history_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("history_id") ?? Route("history_id")) !== null) {
                    $this->history_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->history_id->CurrentValue = null;
                }
            }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("SupportRequestHistoryList"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "SupportRequestHistoryList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SupportRequestHistoryList") {
                            ...
                            $returnUrl = "SupportRequestHistoryList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'history_id' first before field var 'x_history_id'
        $val = $CurrentForm->hasValue("history_id") ? $CurrentForm->getValue("history_id") : $CurrentForm->getValue("x_history_id");
        if (!$this->history_id->IsDetailKey) {
            $this->history_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->history_id->CurrentValue = $this->history_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // history_id
        $this->history_id->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->history_id->Visible && $this->history_id->Required) {
                if (!$this->history_id->IsDetailKey && EmptyValue($this->history_id->FormValue)) {
                    $this->history_id->addErrorMessage(str_replace("%s", $this->history_id->caption(), $this->history_id->RequiredErrorMessage));
                }
            }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SupportRequestHistoryList"), "", $this->TableVar, true);
        $pageId = "edit";
