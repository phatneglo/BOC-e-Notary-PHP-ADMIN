 ...
 * Page class
 */
class NotarizationQueueEdit extends NotarizationQueue
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NotarizationQueueEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "NotarizationQueueEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("queue_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->queue_id->setQueryStringValue($keyValue);
                $this->queue_id->setOldValue($this->queue_id->QueryStringValue);
            } elseif (Post("queue_id") !== null) {
                $this->queue_id->setFormValue(Post("queue_id"));
                $this->queue_id->setOldValue($this->queue_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                if (($keyValue = Get("queue_id") ?? Route("queue_id")) !== null) {
                    ...
                    $this->queue_id->CurrentValue = null;
                }
            }
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->queue_id->IsDetailKey) {
            $this->queue_id->setFormValue($val);
        }
    ...
    {
        global $CurrentForm;
        $this->queue_id->CurrentValue = $this->queue_id->FormValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->queue_id->Visible && $this->queue_id->Required) {
                if (!$this->queue_id->IsDetailKey && EmptyValue($this->queue_id->FormValue)) {
                    $this->queue_id->addErrorMessage(str_replace("%s", $this->queue_id->caption(), $this->queue_id->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (request_id)
        if ($this->request_id->CurrentValue != "") {
            $filterChk = "(\"request_id\" = " . AdjustSql($this->request_id->CurrentValue, $this->Dbid) . ")";
    ...
    protected function getEditRow($rsold)
    {
        ...
        // queue_position
        $this->queue_position->setDbValueDef($rsnew, $this->queue_position->CurrentValue, $this->queue_position->ReadOnly);

        // entry_time
        $this->entry_time->setDbValueDef($rsnew, UnFormatDateTime($this->entry_time->CurrentValue, $this->entry_time->formatPattern()), $this->entry_time->ReadOnly);

        // processing_started_at
        $this->processing_started_at->setDbValueDef($rsnew, UnFormatDateTime($this->processing_started_at->CurrentValue, $this->processing_started_at->formatPattern()), $this->processing_started_at->ReadOnly);

        // completed_at
        $this->completed_at->setDbValueDef($rsnew, UnFormatDateTime($this->completed_at->CurrentValue, $this->completed_at->formatPattern()), $this->completed_at->ReadOnly);

        // status
        $this->status->setDbValueDef($rsnew, $this->status->CurrentValue, $this->status->ReadOnly);

        // estimated_wait_time
        $this->estimated_wait_time->setDbValueDef($rsnew, $this->estimated_wait_time->CurrentValue, $this->estimated_wait_time->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['queue_position'])) { // queue_position
            $this->queue_position->CurrentValue = $row['queue_position'];
        }
        if (isset($row['entry_time'])) { // entry_time
            $this->entry_time->CurrentValue = $row['entry_time'];
        }
        if (isset($row['processing_started_at'])) { // processing_started_at
            $this->processing_started_at->CurrentValue = $row['processing_started_at'];
        }
        if (isset($row['completed_at'])) { // completed_at
            $this->completed_at->CurrentValue = $row['completed_at'];
        }
        if (isset($row['status'])) { // status
            $this->status->CurrentValue = $row['status'];
        }
        if (isset($row['estimated_wait_time'])) { // estimated_wait_time
            $this->estimated_wait_time->CurrentValue = $row['estimated_wait_time'];
        }
    }
