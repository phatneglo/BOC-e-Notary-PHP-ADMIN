...
?>
<main class="edit">
<form name="fdocument_verificationedit" id="fdocument_verificationedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { document_verification: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fdocument_verificationedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fdocument_verificationedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["verification_id", [fields.verification_id.visible && fields.verification_id.required ? ew.Validators.required(fields.verification_id.caption) : null], fields.verification_id.isInvalid],
...
<?php if ($Page->verification_id->Visible) { // verification_id ?>
    <div id="r_verification_id"<?= $Page->verification_id->rowAttributes() ?>>
        <label id="elh_document_verification_verification_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->verification_id->caption() ?><?= $Page->verification_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->verification_id->getDisplayValue($Page->verification_id->EditValue))) ?>"></span>
<input type="hidden" data-table="document_verification" data-field="x_verification_id" data-hidden="1" name="x_verification_id" id="x_verification_id" value="<?= HtmlEncode($Page->verification_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fdocument_verificationedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdocument_verificationedit", "x_expiry_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fdocument_verificationedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdocument_verificationedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fdocument_verificationedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fdocument_verificationedit", "x_blocked_until", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdocument_verificationedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdocument_verificationedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
