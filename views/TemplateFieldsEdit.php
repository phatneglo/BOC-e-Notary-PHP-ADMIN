...
?>
<main class="edit">
<form name="ftemplate_fieldsedit" id="ftemplate_fieldsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { template_fields: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var ftemplate_fieldsedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("ftemplate_fieldsedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["field_id", [fields.field_id.visible && fields.field_id.required ? ew.Validators.required(fields.field_id.caption) : null], fields.field_id.isInvalid],
...
<?php if ($Page->field_id->Visible) { // field_id ?>
    <div id="r_field_id"<?= $Page->field_id->rowAttributes() ?>>
        <label id="elh_template_fields_field_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->field_id->caption() ?><?= $Page->field_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->field_id->getDisplayValue($Page->field_id->EditValue))) ?>"></span>
<input type="hidden" data-table="template_fields" data-field="x_field_id" data-hidden="1" name="x_field_id" id="x_field_id" value="<?= HtmlEncode($Page->field_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["ftemplate_fieldsedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ftemplate_fieldsedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ftemplate_fieldsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ftemplate_fieldsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
