...
?>
<main class="edit">
<form name="fuser_templatesedit" id="fuser_templatesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { user_templates: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fuser_templatesedit;
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("fuser_templatesedit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["user_template_id", [fields.user_template_id.visible && fields.user_template_id.required ? ew.Validators.required(fields.user_template_id.caption) : null], fields.user_template_id.isInvalid],
...
<?php if ($Page->user_template_id->Visible) { // user_template_id ?>
    <div id="r_user_template_id"<?= $Page->user_template_id->rowAttributes() ?>>
        <label id="elh_user_templates_user_template_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->user_template_id->caption() ?><?= $Page->user_template_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->user_template_id->cellAttributes() ?>>
<span id="el_user_templates_user_template_id">
<span<?= $Page->user_template_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->user_template_id->getDisplayValue($Page->user_template_id->EditValue))) ?>"></span>
<input type="hidden" data-table="user_templates" data-field="x_user_template_id" data-hidden="1" name="x_user_template_id" id="x_user_template_id" value="<?= HtmlEncode($Page->user_template_id->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fuser_templatesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fuser_templatesedit", "x_created_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fuser_templatesedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fuser_templatesedit", "x_updated_at", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fuser_templatesedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fuser_templatesedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
